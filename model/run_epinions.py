...
logging.basicConfig(level=logging.DEBUG #设置日志输出格式
                    ,filename="./epinions_res.log" #log日志输出的文件位置和文件名
...
def train(g, test_g, mean_rate, args):
    ...
    # g.nodes['iid'].data['trust'] = emb_dict['item_emb']
    ...
    opt = torch.optim.Adam([{'params': model.parameters()}], lr=0.0005, weight_decay=0.015)  # 惩罚系数越小 0.005  cold 惩罚系数大一些
    # epinions  0.0005 train 0.972922189368142, MAE is 0.7487
    dur1 = []
    ...
    for epoch_id in range(1, args.num_epochs + 1):
        ...
        for input_nodes, subgraph, neg, blocks in dataloader:
            ...
            trust_features = subgraph.srcdata['trust']  # srcdata
            ...
            rating_loss, mae, loss_reg, pos_score, l_att, l_a_x, trust_auc, trust_ap, loss_trust = \
                model(subgraph, neg, blocks, input_features['uid'], input_features['iid'], trust_features['uid'],
                      0.001, a, helpfulness)  # , trust_auc, trust_ap
            # loss优化
            opt.zero_grad()  # 试试
            # (rating_loss + mae + 0.4*loss_trust).backward()
            (torch.pow(rating_loss, 2) + 0.015 * (torch.pow(loss_reg, 2)) + l_a_x + 1 * torch.pow(loss_trust, 2) + mae +
             1 * torch.pow(l_att, 2)).backward()
            # 0.1 * loss_trust  0.8*l_a_x  mae
            opt.step()
...
def test(g, model, mean_rate, args):
    ...
    """
    dur1 = []
    emb = torch.load(f'../ckpt/epinions_92_emb.pt')
    g.ndata['emb'], g.ndata['a'] = emb['rating'], emb['attitude']
    ...
    if not model:
        ...
        model.load_state_dict(torch.load(f'../ckpt/epinions_92_model.pt'))  # 调用最好的 model
    model.eval()
    with torch.no_grad():
        count_rmse = 0
        count_mae = 0
        ...
        dur1.append(t1 - t0)
        print(
            f'Test Rating Prediction: RMSE is {count_rmse / n}, MAE is {count_mae / n}, att_loss is {count_att / n}')
...
def val(g, model, mean_rate, args, x, a):
    ...
    dataloader = dgl.dataloading.DataLoader(
        g, test_nid, sampler,
        batch_size=10480, shuffle=True, drop_last=False)
    ...
    with torch.no_grad():
        ...
        # cold.to_csv('cold_epinions.txt', index=0, header=0)
        t1 = time.time()
        dur1.append(t1 - t0)
        print(
...
def main():
    ...
    parser.add_argument('--dataset_path', type=str, default='../dataset/epinions_7411_8728/dataset.pkl')
